...
4. **Call the `serverStatus` endpoint** to verify the node is accessible:
   ...
   - `<GRPC_PORT>` is the server-status port exposed by your Block Node. In the LFH profile this is `40982` (`PORTS_SERVER_STATUS`); in a base-chart deployment it is `40840` (see `server.port` in [configuration.md](../configuration.md)).
