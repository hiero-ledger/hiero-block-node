...
> **Note:** Always use the latest published simulator image from the [package registry](https://github.com/hiero-ledger/hiero-block-node/pkgs/container/hiero-block-node%2Fsimulator-image).
...
3. **Using a gRPC client (optional)**: 

- In a separate terminal on your Block Node VM, run grpcurl against the same address and port:

  ```bash
  grpcurl -plaintext 10.96.0.15:40840 list
  ```

  Expected output:
  
    ```bash
    hiero.block.api.BlockNodeService
    ```
- Use Postman to call specific RPCs if you want to validate additional behavior.
