...
A JSON file containing the RSA public keys of the genesis Consensus Node roster, used to verify
[Wrapped Record Block](#wrb-wrapped-record-block) proofs during WRB streaming (before TSS
signatures are available). Loaded by the Block Node at startup.
Configured via `app.state.rsaBootstrapFilePath`; default path:
`/opt/hiero/block-node/application-state/rsa-bootstrap-roster.json`.
Required for Tier 1 nodes performing WRB verification. The file must be accessible inside the
container when running Docker or Kubernetes deployments.
