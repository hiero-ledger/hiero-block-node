...
import static com.hedera.block.server.metrics.BlockNodeMetricTypes.Counter.OutboundStreamingError;
 ...
 * client.
 */
class OpenRangeStreamManager implements StreamManager {
    ...
    // The state machine for managing the transitions between live-streaming and historic-streaming
    enum State {
        INIT_HISTORIC {
            @NonNull
            @Override
                ...
                if (m.isLiveStreamInitialized.get()) {
                    ...
                    if (historicStreamBlockNumber == liveStreamBlockNumber) {
                        ...
                        LOGGER.log(
                                ...
                                m.managerId,
                                liveStreamBlockNumber);
                        logTransition(State.LIVE_STREAMING);
        ...
        CUE_LIVE_STREAMING {
            ...
            public State execute(@NonNull final OpenRangeStreamManager m) throws Exception {
                ...
                logTransition(State.CUE_LIVE_STREAMING);
        ...
        LIVE_STREAMING {
            ...
            public State execute(@NonNull final OpenRangeStreamManager m) throws Exception {
                ...
                if (liveDataOpt.isPresent()) {
                    ...
                    // send data to the client
                    m.consumerStreamResponseObserver.send(liveDataOpt.get().get());
        ...
        public void logTransition(State next) {
            LOGGER.log(TRACE, "Current state: {0} ==> {1}", this, next);
        }
    }
