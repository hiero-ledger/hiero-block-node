 ...
 *
 * <p>The plugin keeps more than one version of state alive at once. The gRPC
 * query API only ever reads {@link #IMMUTABLE}; the other sources exist so that
 * in-process callers (future plugin-to-plugin reads via a {@code BinaryStateReader}
