...
public class SubscriberServicePlugin implements BlockNodePlugin, ServiceInterface {
    ...
    static class SubscribeBlockStreamHandler implements ServerStreamingMethod<
                SubscribeStreamRequest, SubscribeStreamResponseUnparsed> {
        private final Logger LOGGER = System.getLogger(getClass().getName());
        ...
        private void stop() {
            ...
            for (final BlockStreamSubscriberSession session : openSessions.values()) {
                session.close(SubscribeStreamResponseCode.READ_STREAM_SUCCESS);
            }
        ...
        @Override
        public void apply(@NonNull final SubscribeStreamRequest request, @NonNull final Pipeline<? super SubscribeStreamResponseUnparsed> responsePipeline) throws InterruptedException {
            final long clientId = nextClientId.getAndIncrement();
            final CountDownLatch sessionReadyLatch= new CountDownLatch(1);
            final BlockStreamSubscriberSession blockStreamSession = new BlockStreamSubscriberSession(clientId, request, responsePipeline, context, sessionReadyLatch);
        ...
        private void handleCompletedStream(final Future<BlockStreamSubscriberSession> completedSessionFuture) throws InterruptedException {
            try {
                ...
                final String message =
                        "Subscriber session failed due to unhandled %s:%n{0}.".formatted(e.getCause());
                LOGGER.log(Level.ERROR, message, e);
            }
