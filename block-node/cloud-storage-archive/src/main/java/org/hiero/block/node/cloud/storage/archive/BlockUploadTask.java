    ...
    private byte[] flushPartIfNeeded(
        ...
        try {
            ...
            if (buffer.length >= partSizeBytes) {
                ...
                applicationStateFacility.addBlockRange(new LongRange(rangeStart, last.getKey()), BlockRangeType.STORED);
    ...
    private UploadResult uploadFinalPart(
        ...
        if (partResult == UploadResult.SUCCESS) {
            ...
            applicationStateFacility.addBlockRange(
                    new LongRange(blocksInBuffer.firstKey(), last.getKey()), BlockRangeType.STORED);
            metricsHolder.blocksWritten().increment(blocksInBuffer.size());
