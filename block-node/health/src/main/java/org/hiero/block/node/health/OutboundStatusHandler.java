...
/**
 * Builds and sends the {@code /statusz/outbound} response: the merge of the outbound designated
 * partners and the backfill sources, serialized as {@link NetworkData} JSON.
 *
...
final class OutboundStatusHandler {
    ...
    /** Builds the outbound {@link NetworkData} and sends it as an {@code application/json} response. */
    void createAndSendResponse() {
        ...
        LOGGER.log(
                TRACE,
                "Responded to {0} with {1} outbound endpoints",
                request.path().path(),
                endpoints.size());
    }
}
