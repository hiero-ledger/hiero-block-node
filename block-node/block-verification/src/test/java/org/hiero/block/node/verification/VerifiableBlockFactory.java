...
/// Factory for creating synthetic blocks that pass `ExtendedMerkleTreeSession` verification.
///
/// Blocks use the hash-of-hash signature path: the proof signature is `SHA384(blockRootHash)`.
/// HAPI version 1.2.3 routes to `ExtendedMerkleTreeSession` (>= 0.72.0).
final class VerifiableBlockFactory {
    ...
    /// the `previousBlockHash` argument when creating the next block in the chain.
    ///
    /// @param blockNumber the block number
    /// @param previousBlockHash the hash of the previous block, or null for genesis
