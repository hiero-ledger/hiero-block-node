 ...
 *
 * @param flowControlTimeout Outbound flow control blocking timeout in milliseconds. Blocking timeout defines an interval to wait for the outbound window size changes(incoming window updates). When exceeded, Helidon sends GOAWAY and closes the whole connection, so this must tolerate real-network stalls (RTT, GC pauses, TCP retransmits). Helidon default is 15s.
 ...
 * @param maxConcurrentStreams Maximum number of concurrent streams that the server will allow per connection. Helidon default is 8192. Exceeding it causes Helidon to send GOAWAY and close the whole connection. This limit is directional: it applies to the number of streams that the sender permits the receiver to create. It is recommended that this value be no smaller than 100 to not unnecessarily limit parallelism See RFC 9113 section 6.5.2 for details.
...
public record WebServerHttp2Config(
        @Loggable @ConfigProperty(defaultValue = "10_000") int flowControlTimeout,
