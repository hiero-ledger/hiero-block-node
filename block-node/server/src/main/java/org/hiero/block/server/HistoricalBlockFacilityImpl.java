...
import org.hiero.block.server.plugins.historicalblocks.BlockAccessor;
import org.hiero.block.server.plugins.historicalblocks.BlockProviderPlugin;
...
public class HistoricalBlockFacilityImpl implements HistoricalBlockFacility {
    ...
    public HistoricalBlockFacilityImpl(final Configuration configuration) {
        ...
        providers = ServiceLoader.load(BlockProviderPlugin.class, getClass().getClassLoader())
                .stream()
                .map(Provider::get)
                .sorted(Comparator.comparingInt(BlockProviderPlugin::defaultPriority).reversed())
                .toList();
    }
