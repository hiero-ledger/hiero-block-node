...
class StreamPublisherPluginTest {
    ...
    class PluginTestsFailedVerification
        ...
        /// This test aims to assert that a failed verification of a possibly transient type keeps the
        /// supplying stream open: no EndOfStream is sent, but the failed block is still scheduled for a
        /// resend. When an active publisher finishes its current block, it must receive the ResendBlock
        /// message for the failed block.
        @ParameterizedTest
        @EnumSource(
                value = FailureType.class,
                names = {"MISSING_VERIFICATION_DATA", "UNABLE_TO_PARSE"})
        @DisplayName("Test no EndOfStream but ResendBlock on possibly transient failure types, stream stays open")
        ...
        /// This test aims to assert that a failed verification of type CANCELLED is only logged: no
        /// EndOfStream is sent, no resend is scheduled, and both publishers stay open and receive the
        /// acknowledgement for the next persisted block. A cancelled session also warrants a resend, but it
        /// cannot be scheduled accurately until @todo(3436) is in place.
        @Test
        @DisplayName("Test no responses at all on a CANCELLED failure, both streams stay open")
        void testCancelledNoAction() {
            // Tell the verification plugin to fail block 1 with the CANCELLED failure type.
            verificationPlugin.setFailureType(FailureType.CANCELLED);
            verificationPlugin.failBlocks(1L);
            // Create a second publisher, the first one is automatically created by the plugin test base
            final TestPipeline secondPublisher = createNewPipeline();
            final List<List<Bytes>> ackReceivers = List.of(fromPluginBytes, secondPublisher.fromPluginBytes());
