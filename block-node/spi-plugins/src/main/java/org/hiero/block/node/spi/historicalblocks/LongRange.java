...
public record LongRange(long start, long end) implements Comparable<LongRange> {
    ...
    private static final String INVALID_VALUES_TO_COMPARE_MSG =
            "Provided start: %d value to compare must be equal to or lower than provided end: %d value to compare";
    ...
    public boolean contains(final long start, final long end) {
        if (start > end) {
            throw new IllegalArgumentException(INVALID_VALUES_TO_COMPARE_MSG.formatted(start, end));
        } else {
