...
public class ResourceTestBlockBuilder {
    ...
    /// TSS WRAPS test blocks. Only BLOCK_0 is retained as canary for
    /// {@code BlockHasherTest.PositiveBlockHasher} — the "our hasher matches the real-CN
    /// captured hash" invariant. All non-hasher WRAPS tests generate their blocks via the
    /// harness ({@code HarnessChainBuilder}); the E2E lifecycle workflow generates its
    /// blocks at CI time via {@code :block-verification:generateHarnessBlocks}.
    public enum WRAPS implements ResourceBlock {
        /// Genesis block — bootstraps TSS parameters and ledger ID. Canary only.
        BLOCK_0(
