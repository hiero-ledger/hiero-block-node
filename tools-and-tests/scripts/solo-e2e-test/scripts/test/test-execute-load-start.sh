...
#
# Regression check for execute_load_start's NLG_TEST_TYPE wiring in solo-test-runner.sh.
#
# A test definition's `load-start` event accepts a `test_class` arg (e.g.
# HCSLoadTest) and execute_load_start read it, defaulted it, and even echoed it
# in its log line — but never exported NLG_TEST_TYPE before invoking
# solo-load-generate.sh, which selects its class from
# `${NLG_TEST_TYPE:-CryptoTransferLoadTest}`. So test_class was silently
# discarded: every in-test load-start always ran CryptoTransferLoadTest
# regardless of what a test YAML asked for, no matter what
# workflow_dispatch's nlg-test-type input was set to (that input only reaches
# the separate, standalone pre-test NLG step, not this one).
#
# Runs without a cluster: extracts execute_load_start via sed (same approach
# as test-topology-decisions.sh) rather than sourcing the whole runner script,
...
# Reuse the real function rather than a copy, so this test tracks the
