...
function extract_block_hashes_from_records {
    ...
    python3 - "${records_dir}" "${output_file}" <<'PYTHON_EOF'
import sys
import os
import gzip
import struct
...
function generate_metadata_from_filelist {
    ...
    python3 - "${LOCAL_RECORDS_DIR}" "${block_times_file}" "${day_blocks_file}" "${genesis_epoch_nanos}" <<'PYTHON_EOF'
import sys
import os
import gzip
import struct
...
function update_day_blocks_with_hashes {
    ...
    python3 - "${day_blocks_file}" "${hash_file}" <<'PYTHON_EOF'
import sys
import json
