...
function execute_deploy_blocknode {
    ...
    # nameOverride/fullnameOverride ensure the StatefulSet and pod labels match
    # what the runner's assert functions expect (app.kubernetes.io/name=<bn_name>),
    # matching how Solo CLI deploys BN1/BN2.
    cat > "${values_file}" << EOF
# Generated by solo-test-runner.sh for dynamic block node deployment
nameOverride: "${bn_name}"
fullnameOverride: "${bn_name}"
blockNode:
  config:
...
EOF
    ...
    # Apply the same memory limits as static BNs (8Gi request, 20Gi limit) so the dynamic
    # BN can be scheduled on a memory-constrained Kind cluster after another BN is killed.
    # Without this, the chart defaults (12Gi request) exceed the freed memory budget.
    local memory_override_args=""
    local memory_override="${SCRIPT_DIR}/../overrides/bn-memory.yaml"
    if [[ -f "${memory_override}" ]]; then
