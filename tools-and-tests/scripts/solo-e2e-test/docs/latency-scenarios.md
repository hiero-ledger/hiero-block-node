...
- **Tests are predicated on a healthy cluster.** The `paired-3` cluster has been observed to occasionally enter a stalled state where consensus stops producing blocks (CN-1 restart in one case; freshly-brought-up cluster with consensus already wedged at a low block in another). When that happens, `block-rate-floor` reports `0.000/s` — that's the assertion correctly detecting the upstream problem, not a test flake. Sanity-check `task verify` (or `kubectl exec -n solo-network network-node1-0 -c root-container -- cat …` for HAPI logs) before chalking a latency-test failure up to the test itself. Reliable signal we have today: 8 of 8 latency tests pass back-to-back on a healthy cluster (`iter1` + `iter2` of a 3× sweep). The cluster-stability issue is upstream of the chaos work and is being investigated separately.
