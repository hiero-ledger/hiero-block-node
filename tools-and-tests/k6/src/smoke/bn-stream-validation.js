...
export default async () => {
    ...
    console.log(
        `First Available Block: ${firstAvailableBlock}, Latest Block: ${lastAvailableBlock}`
    );
    ...
    if (response.message.firstAvailableBlock === "18446744073709551615") {
        ...
        lastAvailableBlock - firstAvailableBlock <
        data.smokeTestConfigs.numOfBlocksToStream
    ) {
        ...
        console.log(
            `Block Node has sufficient blocks to stream ${data.smokeTestConfigs.numOfBlocksToStream} blocks.`
        );
    }
    ...
    const stream = new SubscribeBlockStreamRequest(client).invoke(
        subscribeParams
    );
    ...
    const streamEnded = new Promise((resolve, reject) => {
        stream.on("data", (subscribeStreamResponse) => {
            if (subscribeStreamResponse.blockItems) {
                ...
                const header =
                    subscribeStreamResponse.blockItems.blockItems[0]
                        .blockHeader;
                if (header) {
                ...
                if (subscribeStreamResponse.status !== `SUCCESS`) {
                    console.log(
                        `Received Non-Success Status: ${subscribeStreamResponse.status}`
                    );
                } else {
                ...
                console.log(
                    `Unknown Stream Response: , ${JSON.stringify(
