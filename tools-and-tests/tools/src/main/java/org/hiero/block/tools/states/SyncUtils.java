...
public class SyncUtils {
	...
	static void writeInstant(DataOutput dos, Instant time, int[] byteCount)
			throws IOException {
	...
	static void writeLongArray(DataOutput dos, long[] data, int[] byteCount)
			throws IOException {
	...
	static long[] readLongArray(DataInput dis, int[] byteCount, int maxArrayLength)
			throws IOException {
		int len = dis.readInt();
		if (len < 0) {
			throw new BadIOException(
					"SyncServer.readByteArray2d tried to create array of length "
							+ len);
		}
	 ...
	 * the number of bytes
	 */
	static AtomicLongArray readAtomicLongArray(DataInput dis, int[] byteCount, int maxArrayLength)
			throws IOException {
		int len = dis.readInt();
		if (len < 0) {
			throw new BadIOException(
	...
	static void writeByteArray(DataOutputStream dos, byte[] data,
	 ...
	 */
	static void writeByteArray(DataOutputStream dos, int len, int[] byteCount)
			throws IOException {
	...
	static byte[] readByteArray(DataInputStream dis, int[] byteCount, int maxArrayLength)
			throws IOException {
		int len = dis.readInt();
		checkArrayLength(len, maxArrayLength);
	...
	static void writeBooleanArray(DataOutputStream dos, boolean[] data,
			int[] byteCount) throws IOException {
	...
	static boolean[] readBooleanArray(DataInputStream dis, int[] byteCount)
			throws IOException {
	...
	/** write a byte[][] to a data stream and increment byteCount[0] by the number of bytes */
	static void writeByteArray2d(DataOutputStream dos, byte[][] data,
			int[] byteCount) throws IOException {
		dos.writeInt(data.length);
	...
	/** read a byte[][] from a data stream and increment byteCount[0] by the number of bytes */
	static byte[][] readByteArray2d(DataInputStream dis, int[] byteCount)
			throws IOException {
		int len = dis.readInt();
		int checksum = dis.readInt();
